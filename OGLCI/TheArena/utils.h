...
#ifdef WIN32
inline int lowerRound(double x)
{
    if (x > 0)
...
template<class T>
inline Vector2s grad_bilerp(const T& v00, const T& v10,
    const T& v01, const T& v11,
    T fx, T fy)
{
    return Vector2s(fy - 1.0, fx - 1.0) * v00 +
        Vector2s(1.0 - fy, -fx) * v10 +
        Vector2s(-fy, 1.0 - fx) * v01 +
        Vector2s(fy, fx) * v11;
}
